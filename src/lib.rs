...
// Framework-independent re-exports (always available)
pub use audio::AudioBuffer;
pub use models::config::Qwen3TTSConfig;
pub use models::{
    codec_tokens, special_tokens, tts_tokens, Language, ModelType, ParsedModelConfig, Speaker,
    SpeakerEncoderConfig, TalkerConfig,
};

// Burn-based re-exports (always available)
pub use burn_models::facade::{
    Qwen3TTS, StreamingSession, SynthesisOptions, CODEC_EOS_TOKEN_ID, SAMPLES_PER_FRAME,
};
pub use burn_models::sampling::{GenerationConfig, SamplingContext};

// Candle-era re-exports (only with legacy feature)
#[cfg(feature = "hub")]
pub use hub::ModelPaths;
#[cfg(feature = "_candle_legacy")]
pub use models::{CodePredictor, CodePredictorConfig, TalkerModel};
...
// ── Candle-based implementation (gated) ─────────────────────────────────────
//
// Everything below is the original Candle integration layer.
// It does not compile without candle dependencies and will be removed
// once the Burn-based integration (Phase 9) is complete.
#[cfg(feature = "_candle_legacy")]
mod _candle_impl {
    use super::*;
    use anyhow::Result;
    ...
    impl Qwen3TTS {
        ...
        fn generate_codes(
            ...
            let mut semantic_token_tensor =
                generation::sample(&logits_2d, gen_config, sampling_ctx)?;
        ...
        pub fn synthesize_voice_clone_debug(
                ...
                (&prompt.ref_codes, &prompt.ref_text_ids)
            {
                ...
                if icl_len > 0 {
                    let mask =
                        models::transformer::create_causal_mask(icl_len, offset, &self.device)?;
    ...
    mod tests {
        ...
        fn test_suppression_mask_deterministic() {
            ...
            let mask1 =
                generation::build_suppression_mask(vocab, CODEC_EOS_TOKEN_ID, &device).unwrap();
            let mask2 =
                generation::build_suppression_mask(vocab, CODEC_EOS_TOKEN_ID, &device).unwrap();
